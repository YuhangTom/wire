...
get_x3p_inner_impute <- function(x3p, mask_col = "#FF0000", concavity = 1.5) {
  ...
  dir.create(paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name), showWarnings = FALSE)
  ...
  ggsave(paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name, "/", x3p_name, "_gif_p0.png"), p0, width = 5, height = 3)
  ...
  p1
  ggsave(paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name, "/", x3p_name, "_gif_p1.png"), p1, width = 5, height = 3)
  ...
  while (do) {
    ...
    p
    ggsave(paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name, "/", x3p_name, "_gif_p", nimp, ".png"), p, width = 5, height = 3)
  ...
  x3p_inner_nomiss_res_focal_impute_gif <- paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name, "/", x3p_name, "_gif_p", 0:nimp, ".png") %>%
    map(image_read) %>%
  ...
  image_write(
    ...
    path = paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name, "/", x3p_name, "_focal_impute.gif")
  )

  list.files(
    path = paste0("./images/02f-AluminumWires-RasterSquare/", x3p_name),
    full.names = TRUE
  ) %>%
  ...
  x3p_inner_focal_impute <- x3p_inner_nomiss_res %>%
    x3p_delete_mask()
  x3p_inner_focal_impute$surface.matrix <- t(x3p_inner_nomiss_res_focal_raster %>% as.matrix())
